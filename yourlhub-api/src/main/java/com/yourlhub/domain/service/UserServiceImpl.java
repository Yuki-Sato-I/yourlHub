...
public class UserServiceImpl implements UserService {
    ...
    @Autowired
    public UserServiceImpl(final UserRepository userRepository, final PasswordEncoder passwordEncoder, final JwtTokenUtil jwtTokenUtil) {
    ...
    /**
     * emailでユーザーを検索し、パスワード認証を行う。
     * パスワード認証に失敗した場合は例外を投げる。
     *
     * @param input
     * @return
     */
    public Mono<String> findByEmailAndAuthenticatePassword(final AuthInput input) {
        ...
        if (Objects.isNull(user) || !passwordEncoder.matches(input.password(), user.getPasswordDigest())) {
